...
(function($) {
	...
	function drawCalControl (target, opt) {
		...
		$(target).append(
			'<div class="jCal">' + 
					( (opt.ind == 0) ? '<div class="left"><img src="' + opt.css + 'esquerda.gif"></div>' : '' ) + 
					'<div class="month">' + opt.ml[opt.day.getMonth()] + ' ' + opt.day.getFullYear() + '</div>' +
					( (opt.ind == ( opt.showMonths - 1 )) ? '<div class="right"><img src="' + opt.css + 'dereita.gif"></div>' : '' ) +
			'</div>');
	...
	function drawCal (target, opt) {
		...
		var dia = opt.day.getDate();
		for (var d=1; d < ( fd.getDay() + ld.getDate() + ( 7 - ld.getDay() ) ); d++) {
			var clase = (d - fd.getDay()) != dia ? "day" : "day selectedDay";
			$(target).append(
				(( d <= fd.getDay() ) ? 
					...
					: ( ( d > ( fd.getDay() + ld.getDate() ) ) ?
						...
						: '<div id="' + opt.cID + 'd_' + (fd.getMonth() + 1) + '_' + ( d - fd.getDay() ) + '_' + fd.getFullYear() + '" class="' + clase + '">' + ( d - fd.getDay() )  + '</div>'
					)
				)
			);
		}
